...
\begin{example}
    ...
    \begin{align}
        ...
        3x-y &= 2?
    \end{align}
    Then, %\textcolor{red}{how we can write this in matrix form}: 
    let $\vec{x} = [x;y]$, and
    \begin{align}
    A\vec{x} = \vec{b}
    \end{align}
    ...
    Let's try to make this look like (1), (2) by considering the eigendecomposition of $A$.
    
    Matlab Code
    \begin{verbatim}
        A = [7 3; 3 -1]; % how to define a matrix
        % notice that a semicolon suppresses output
...
\begin{example}
   ...
   \begin{align}
       ...
       \end{bmatrix}
   \end{align}
   has an eigenvalue $1$ with algebraic multiplicity $2$, but geometric multiplicity $1$, i.e. it has one eigenvector $[1; 0]$.  Matrices are only invertible if they are full rank, and therefore the matrix of eigenvectors is not invertible.
 \end{example}
